...
def remove_unwanted_substrings(name: str) -> str:
    # 移除指定的干擾字串
    return name.replace("（珍藏老歌 httpblog.sina.com.cnu1766037260）", "").strip()
...
def normalize_path(path: Path) -> Path:
    # 將常見特殊符號轉為安全字元
    safe_name = path.name.translate(str.maketrans({
        '【': '[', '】': ']', '+': '＋'
    }))
    ...
    if path != safe_path:
        try:
            ...
            print(f"📁 已重新命名特殊字元資料夾: {safe_path.name}")
...
def convert_folder_to_traditional(directory: Path):
    for path in sorted(directory.rglob('*'), key=lambda p: -len(str(p))):
        ...
        new_name = remove_unwanted_substrings(new_name)
...
def ffmpeg_split_and_tag(cue_info, cue_path: Path):
    ...
    audio_dir = cue_path.parent
    full_audio_path = audio_dir / audio_file

    if not full_audio_path.exists():
        # 加強容錯：比對音檔檔名不含副檔名、大小寫不敏感
        candidates = list(audio_dir.glob("*.wav"))
        def normalize_name(name):
            return re.sub(r'[^A-Za-z0-9]', '', name).lower()
        target_name = normalize_name(audio_file.rsplit('.', 1)[0])
        for c in candidates:
            if normalize_name(c.stem) == target_name:
                full_audio_path = c
                break
        else:
            if candidates:
                full_audio_path = max(candidates, key=lambda p: p.stat().st_size)
    ...
    for i, track in enumerate(cue_info['tracks']):
        ...
        out_path = get_unique_file_path(audio_dir, f"{cue_info['performer']} - {title}.wav")
        cmd = [
            FFMPEG_PATH,
            '-ss', str(start),
            '-t', f"{duration:.3f}",
            '-i', str(full_audio_path.resolve()),
            '-acodec', 'pcm_s16le',
            '-y', str(out_path.resolve())
        ]
...
def process_cue_file(cue_path: Path):
    ...
    if performer and title and date:
        ...
        new_name = sanitize_folder_name(f"{performer} - {date} - {title}")
